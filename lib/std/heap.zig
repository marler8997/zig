...
var c_allocator_state = Allocator{
    .flags = Allocator.Flags.knowsCapacity | Allocator.Flags.resizeAcceptsPartialLen,
    .allocFn = cAlloc,
    .resizeFn = cResize,
};
...
fn cResize(self: *Allocator, buf: []u8, new_len: usize) usize {
    ...
    if (new_len <= buf.len) {
        // TODO: will realloc guarantee it won't move memory if we are shrinking a buffer?
        return buf.len;
    }
...
const PageAllocator = struct {
    ...
    fn resize(allocator: *Allocator, buf_unaligned: []u8, new_size: usize) usize {
        assert(mem.isAligned(buf_unaligned.len, mem.page_size));
...
pub const FixedBufferAllocator = struct {
    ...
    fn alloc(allocator: *Allocator, n: usize, alignment: u29) ![]u8 {
        const self = @fieldParentPtr(FixedBufferAllocator, "allocator",
            @fieldParentPtr(Allocator, "base", allocator));
        ...
        break :blk struct {
            ...
            pub fn init(buffer: []u8) ThreadSafeFixedBufferAllocator {
                return ThreadSafeFixedBufferAllocator{
                    .allocator = ExactAllocator{.base=.{
                        .allocFn = alloc,
                        .resizeFn = Allocator.exactNoResize,
                    }},
                    .buffer = buffer,
                    .end_index = 0,
                };
            ...
            fn alloc(allocator: *Allocator, n: usize, alignment: u29) Allocator.Error![]u8 {
                const self = @fieldParentPtr(ThreadSafeFixedBufferAllocator, "allocator",
...
test "FixedBufferAllocator.reset" {
    var buf: [8 + Allocator.ExactAllocPadding]u8 align(@alignOf(u64)) = undefined;
...
test "FixedBufferAllocator Reuse memory on realloc" {
    ...
    {
        var buffer: [10 + Allocator.ExactAllocPadding]u8 = undefined;
        var fixed_buffer_allocator = FixedBufferAllocator.init(&buffer);
    ...
    {
        var buffer: [10 + 3 * Allocator.ExactAllocPadding]u8 = undefined;
        var fixed_buffer_allocator = FixedBufferAllocator.init(&buffer);
