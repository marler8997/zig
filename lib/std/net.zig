...
pub const Address = extern union {
    ...
    pub fn parseIp6(name: []const u8, port: u16) !Address {
        return Address{ .in6 = try Ip6Address.parse(name, port) };
    }

    pub fn parseIp4(name: []const u8, port: u16) !Address {
        return Address{ .in = try Ip4Address.parse(name, port) };
    }
    ...
    pub fn getPort(self: Address) u16 {
        switch (self.any.family) {
            os.AF_INET => return self.in.getPort(),
            os.AF_INET6 => return self.in6.getPort(),
            else => unreachable,
        }
    }
    ...
    pub fn initPosix(addr: *align(4) const os.sockaddr) Address {
        switch (addr.family) {
            os.AF_INET  => return Address{ .in  = .{ .sa = @ptrCast(*const os.sockaddr_in , addr).* } },
            os.AF_INET6 => return Address{ .in6 = .{ .sa = @ptrCast(*const os.sockaddr_in6, addr).* } },
            else => unreachable,
        }
    }
