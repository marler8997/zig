...
pub const FailingAllocator = struct {
    ...
    fn alloc(allocator: *std.mem.Allocator, len: usize, alignment: u29) error{OutOfMemory}![]u8 {
        ...
        const result = try self.internal_allocator.callAllocFn(len, alignment);
    ...
    fn resize(allocator: *std.mem.Allocator, buf: []u8, new_len: usize) usize {
        ...
        const result = self.internal_allocator.callResizeFn(buf, new_len);
        if (new_len == 0) {
            self.freed_bytes += buf.len;
        } else if (result < buf.len) {
            self.freed_bytes += buf.len - result;
        } else {
