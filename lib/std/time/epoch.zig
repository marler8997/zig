...
pub fn getDaysInYear(year: anytype) u9 {
...
pub fn getDaysInMonth(year: anytype, month: Month) u5 {
    return switch (month) {
        .jan => 31,
...
pub fn YearAndDay(comptime Year: type) type {
    return struct {
        year: Year,
        ...
        pub fn calculateMonthDay(self: Self) MonthAndDay {
            var month: Month = .jan;
...
/// days since epoch Jan 1, 1970 at 12:00 AM (negative days supported)
fn EpochDay(comptime T: type) type {
    return struct {
        day: T,

        const Self = @This();

        pub fn calculateYearDay(self: Self, comptime Year: type) YearAndDay(Year) {
            if (self.day >= 0) {
            ...
            var day = self.day;
            var year: Year = epoch_year - 1;
            const year_day: u9 = blk: {
                while (true) {
                    const year_size: T = getDaysInYear(year);
                    const diff = year_size + day;
                    if (diff >= 0)
...
/// seconds since epoch Jan 1, 1970 at 12:00 AM (negative seconds supported)
fn EpochSeconds(comptime Seconds: type) type {
    return struct {
        secs: Seconds,
        ...
        /// EpochDay is used to get the calendar day of the month/year.
        pub fn getEpochDay(self: Self) EpochDay(std.math.IntFittingRange(
            @divFloor(std.math.minInt(Seconds), @as(Seconds, secs_per_day)),
            @divFloor(std.math.maxInt(Seconds), @as(Seconds, secs_per_day)),
        )) {
            return .{
                .day = @intCast(@divFloor(self.secs, secs_per_day)),
            };
        }

        /// Returns the number of seconds into the day as DaySeconds.
        /// DaySeconds is used to get the "time of day".
        pub fn getDaySeconds(self: Self) DaySeconds {
            if (self.secs >= 0) return DaySeconds{ .secs = std.math.comptimeMod(self.secs, secs_per_day) };
            const secs_till_next_day = @mod(-self.secs, secs_per_day);
            const day_seconds = if (secs_till_next_day == 0) 0 else secs_per_day - secs_till_next_day;
            return DaySeconds{ .secs = @intCast(day_seconds) };
        }
    };
}

fn testEpoch(secs: anytype, expected_year_day: YearAndDay(u16), expected_month_day: MonthAndDay, expected_day_seconds: struct {
    /// 0 to 23
    hours_into_day: u5,
    /// 0 to 59
    ...
    const epoch_seconds = EpochSeconds(@TypeOf(secs)){ .secs = secs };
    const epoch_day = epoch_seconds.getEpochDay();
    const day_seconds = epoch_seconds.getDaySeconds();
    const year_day = epoch_day.calculateYearDay(u16);
...
test "epoch decoding" {
    try testEpoch(@as(u32, 0), .{ .year = 1970, .day = 0 }, .{
        .month = .jan,
        .day_index = 0,
