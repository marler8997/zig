...
test Moment {
    ...
    const epochs = [_]std.time.Epoch{
        .posix,
        .dos,
        .ios,
        .windows,
    };
    inline for (epochs) |epoch| {
        try testMoment(epoch, .fromInt(u32), .secs);
        try testMoment(epoch, .fromInt(u32), .millis);
...
fn testMoment(
    comptime epoch: std.time.Epoch,
    comptime range: std.time.ComptimeRange,
    comptime unit: std.time.Unit,
) !void {
    try expectMoment(
        Moment(epoch, unit, range){ .offset = 0 },
        .{ .year = epoch.year, .day_index = 0 },
        epoch.dayOfMonth(),
...
fn expectMoment(
    moment: anytype,
    expected_day_of_year: std.time.DayOfYear(u16),
    ...
    const day = moment.asDay();
    const time_of_day = moment.timeOfDay();
    ...
    try testing.expectEqual(expected_day_seconds.hour, time_of_day.hour());
    try testing.expectEqual(expected_day_seconds.minute, time_of_day.minute());
    try testing.expectEqual(expected_day_seconds.second, time_of_day.second());
