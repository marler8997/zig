                    ...
                    .Bool => {
                        ...
                        _ = try self.addInst(.{
                            ...
                            .data = .{ .rr_op = .{
                                .rd = dest_reg,
                                .rn = op_reg,
                                .op = Instruction.Operand.fromU32(1) orelse unreachable,
                            } },
                        });
                ...
                if (int_info.bits <= 16) {
                    ...
                    _ = try self.addInst(.{
                        ...
                        .data = .{ .rr_op = .{
                            .rd = rdlo,
                            .rn = .r0,
                            .op = Instruction.Operand.fromU32(0) orelse unreachable,
                        } },
                    });
...
fn reuseOperand(self: *Self, inst: Air.Inst.Index, operand: Air.Inst.Ref, op_index: Liveness.OperandInt, mcv: MCValue) bool {
    ...
    switch (mcv) {
        .register => |reg| {
            ...
            // for its index
            const index = RegisterManager.indexOfRegIntoTracked(reg) orelse unreachable; // see note above
            if (!self.register_manager.isRegFree(reg)) {
...
fn binOpImmediate(
    ...
    const mir_data: Mir.Inst.Data = switch (mir_tag) {
        ...
        => .{ .rr_op = .{
            .rd = dest_reg,
            .rn = lhs_reg,
            .op = Instruction.Operand.fromU32(rhs.immediate) orelse unreachable,
        } },
        .lsl,
        .asr,
        .lsr,
...
fn airRetLoad(self: *Self, inst: Air.Inst.Index) !void {
    ...
    switch (self.ret_mcv) {
        ...
        .stack_offset => {
            ...
            // location.
            const op_inst = Air.refToIndex(un_op) orelse unreachable;
...
fn lowerDeclRef(self: *Self, tv: TypedValue, decl_index: Module.Decl.Index) InnerError!MCValue {
    ...
    if (self.bin_file.cast(link.File.Elf)) |elf_file| {
        ...
        const got_addr = p9.bases.data + decl.link.plan9.got_index orelse unreachable * ptr_bytes;
        return MCValue{ .memory = got_addr };
    } else {
            ...
            if (typed_value.val.castTag(.enum_field_index)) |field_index| {
                switch (typed_value.ty.tag()) {
                    ...
                    .enum_full, .enum_nonexhaustive => {
                        const enum_full = typed_value.ty.cast(Type.Payload.EnumFull) orelse unreachable.data;
