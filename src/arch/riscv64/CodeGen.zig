...
fn binOpRegister(
    ...
    const lhs_reg = if (lhs_is_register) lhs.register else blk: {
        const track_inst: ?Air.Inst.Index = if (maybe_inst) |inst| inst: {
            const bin_op = self.air.instructions.items(.data)[inst].bin_op;
            break :inst Air.refToIndex(bin_op.lhs) orelse unreachable;
        } else null;
    ...
    const rhs_reg = if (rhs_is_register) rhs.register else blk: {
        const track_inst: ?Air.Inst.Index = if (maybe_inst) |inst| inst: {
            const bin_op = self.air.instructions.items(.data)[inst].bin_op;
            break :inst Air.refToIndex(bin_op.rhs) orelse unreachable;
        } else null;
...
fn airBlock(self: *Self, inst: Air.Inst.Index) !void {
    ...
    for (self.blocks.getPtr(inst) orelse unreachable.relocs.items) |reloc| try self.performReloc(reloc);
...
fn genTypedValue(self: *Self, typed_value: TypedValue) InnerError!MCValue {
    ...
    switch (typed_value.ty.zigTypeTag()) {
        ...
        .ErrorUnion => {
            ...
            const sub_val = typed_value.val.castTag(.eu_payload) orelse unreachable.data;
